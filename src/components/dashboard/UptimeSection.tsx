...
export function UptimeSection({ project }: { project: ProjectDTO }) {
  ...
  async function saveRecipients(body: { emails?: string[]; useReportEmail?: boolean }) {
    ...
    if (!res.ok) {
      setEmailError(json.error || "Correo invalido");
      return false;
    }
    load();
    return true;
  ...
  async function handleTest() {
    ...
    try {
      ...
      setTestResult(`Enviamos 2 correos de prueba a ${json.to} ✓`);
    } catch (err) {
        ...
        <div className="mt-4 pl-7 flex flex-col gap-4">
          <div>
            <div className="flex items-center justify-between flex-wrap gap-2 mb-2">
              <p className="text-xs">
                ...
                {data?.uptimeLastCheckedAt && (
                  <span className="text-neutral-400"> · ultima revision {fmt(data.uptimeLastCheckedAt)}</span>
                )}
              </p>
                ...
                {RANGES.map((r) => (
                  <button
                    ...
                    className={`px-2.5 py-1 rounded-md transition-colors ${
                      range === r.value ? "bg-white shadow-sm text-neutral-900 font-medium" : "text-neutral-500"
                    }`}
                  >
                    {r.label}
                  </button>
            ...
            <div className="flex items-stretch gap-[2px] h-9">
              {(data?.buckets ?? []).map((b) => {
                const color =
                  b.checks === 0 ? "bg-neutral-200" : b.failures > 0 ? "bg-red-500" : "bg-emerald-500";
                const label =
                  b.checks === 0
                    ? "Sin datos"
                    : b.failures > 0
                    ? `${b.failures} de ${b.checks} revisiones fallaron`
                    : `Todo en linea (${b.checks} revisiones)`;
                return (
                  <div
                    key={b.start}
            ...
            <div className="flex items-center gap-4 mt-2 text-xs text-neutral-500 flex-wrap">
              ...
              <span className="flex items-center gap-3 text-neutral-400 ml-auto">
                <span><span className="inline-block w-2 h-2 rounded-sm bg-emerald-500 mr-1" />En linea</span>
                <span><span className="inline-block w-2 h-2 rounded-sm bg-red-500 mr-1" />Caida</span>
                <span><span className="inline-block w-2 h-2 rounded-sm bg-neutral-200 mr-1" />Sin datos</span>
              </span>
            </div>
          </div>

          <div>
            <p className="text-xs font-medium text-neutral-700 mb-1.5">Enviar alertas a</p>
            <label className="flex items-center gap-2 text-xs text-neutral-600 cursor-pointer mb-2">
              ...
              />
              El mismo correo de los reportes (Notificaciones)
            </label>
            ...
            {data && data.recipients.length === 0 ? (
              ...
              <p className="text-xs text-neutral-400 mt-1.5">Se enviara a: {data.recipients.join(", ")}</p>
            ) : null}
          </div>
