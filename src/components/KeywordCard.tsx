      ...
      >
        ...
        <td className="px-2 py-2 min-w-0">
          <div className="flex items-center gap-1.5 min-w-0">
            ...
            <span className="text-[10px] uppercase tracking-wide bg-neutral-100 text-neutral-500 rounded px-1.5 py-0.5 shrink-0">
              {ENGINE_LABELS[keyword.engine] || keyword.engine}
            </span>
            {keyword.source === "planning" && (
              <span
                title="Agregada desde Planificacion"
                className="text-[10px] uppercase tracking-wide bg-blue-50 text-[#1A73E8] rounded px-1.5 py-0.5 shrink-0"
              >
                Planificacion
              </span>
            )}
            {keyword.source === "gsc" && (
              <span
                ...
                className="text-[10px] uppercase tracking-wide bg-blue-50 text-blue-600 rounded px-1.5 py-0.5 shrink-0"
              >
                GSC
              </span>
            )}
        ...
        <tr onClick={(e) => e.stopPropagation()} className="cursor-default">
          <td colSpan={8} className="px-3 pb-2.5 pt-1 bg-neutral-50/50">
            <div className="bg-neutral-50 border border-neutral-100 rounded-lg px-3 py-2.5 flex flex-col gap-2">
              <div>
                <p className="text-[10px] text-neutral-400 uppercase tracking-wide">Tu URL posicionada</p>
                {ownLatest?.url ? (
                  <a
                    href={ownLatest.url}
                    target="_blank"
                    ...
                    {better.map((c) => (
                      <div key={c.domain} className="flex items-center justify-between gap-3 text-xs">
                        <div className="min-w-0 flex items-center gap-2">
                          ...
                          {c.isCompetitor && (
                            <span className="text-[9px] uppercase tracking-wide bg-blue-50 text-[#1A73E8] rounded px-1 py-0.5 shrink-0">
                              Competidor
                            </span>
                          )}
                          {c.url && (
      ...
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span className="text-[11px] uppercase tracking-wide bg-neutral-100 text-neutral-500 rounded px-1.5 py-0.5">
              {ENGINE_LABELS[keyword.engine] || keyword.engine}
            </span>
            ...
            >
              ...
              {r?.url && (
                <a
                  ...
                  rel="noopener noreferrer"
                  title={r.url}
                  className="block text-[11px] text-neutral-400 hover:text-[#1A73E8] truncate mt-0.5"
                >
                  {r.url}
                </a>
              )}
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-1">
        <label className="text-[11px] text-neutral-500">Dominio</label>
        <select
          value={domain}
      ...
      {isAiEngine ? (
        ...
        <div className="flex flex-col gap-1">
          <label className="text-[11px] text-neutral-500">Posicion</label>
          <input
            type="number"
            min={1}
            value={position}
      ...
      <div className="flex flex-col gap-1">
        <label className="text-[11px] text-neutral-500">Fuente</label>
        <select
          value={source}
