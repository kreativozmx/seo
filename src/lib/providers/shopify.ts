...
// Blog article titles from Shopify's public storefront JSON — used so the
// AI content-idea generator can avoid suggesting topics the store already
// covered. Blog handles vary per store ("news", "blog", "noticias"...), so
// we find them from the sitemap instead of guessing.
export async function fetchExistingBlogTitles(domain: string, maxArticles = 200): Promise<string[]> {
  const res = await fetch(`https://${domain}/sitemap.xml`, { headers: BROWSER_HEADERS }).catch(() => null);
  if (!res || !res.ok) return [];
  const rootXml = await res.text();

  const blogSitemapLocs = Array.from(
    rootXml.matchAll(/<loc>\s*([^<\s]+sitemap_blogs[^<\s]*)\s*<\/loc>/gi),
    (m) => m[1]
  );

  const handles = new Set<string>();
  for (const sitemapUrl of blogSitemapLocs) {
    const childRes = await fetch(sitemapUrl, { headers: BROWSER_HEADERS }).catch(() => null);
    if (!childRes || !childRes.ok) continue;
    const childXml = await childRes.text();
    for (const m of Array.from(childXml.matchAll(/<loc>\s*[^<\s]*\/blogs\/([^/<\s]+)\/[^<\s]*\s*<\/loc>/gi))) {
      handles.add(m[1]);
    }
  }

  // No sitemap-derived handle found — "news" is Shopify's default blog
  // handle, worth trying before giving up.
  if (handles.size === 0) handles.add("news");

  const titles: string[] = [];
  for (const handle of Array.from(handles)) {
    if (titles.length >= maxArticles) break;
    const { ok, json } = await fetchJson(
      `https://${domain}/blogs/${handle}/articles.json?limit=250`
    );
    if (!ok) continue;
    const articles: ShopifyArticle[] = (json as { articles?: ShopifyArticle[] })?.articles ?? [];
